...
# Function to apply two 1D Gaussian filters
def apply_1d_gaussian(image, kernel_size, sigma):
    gaussian_1d = cv2.getGaussianKernel(kernel_size, sigma)
    gaussian_1d_h = cv2.filter2D(image, -1, gaussian_1d)
    return cv2.filter2D(gaussian_1d_h, -1, gaussian_1d.T)
...
def main():
    # Parameters
    image_sizes = [512, 1024, 2048, 4096, 8192]  #  Varying image sizes for image size tests
    kernel_sizes = [3, 5, 9, 15, 21, 31, 41]    # Varying kernel sizes for kernel size tests
    ...
    for size in image_sizes:
        ...
        print(f"Image Size: {size}x{size}, 2D Time: {time_2d:.6f}s, 1D x2 Time: {time_1d:.6f}s")

    # Save results to a CSV file
    save_results_to_csv("gaussian_filter_image_sizes.csv", results_image_sizes,
    ...
    for kernel_size in kernel_sizes:
        ...
        # Measure time for 2D Gaussian
        time_2d = measure_time(lambda: apply_2d_gaussian(large_image, kernel_size, sigma), iterations)

        # Measure time for 1D Gaussian applied twice
        time_1d = measure_time(lambda: apply_1d_gaussian(large_image, kernel_size, sigma), iterations)

        # Save results
        results_kernel_sizes.append((kernel_size, time_2d, time_1d))
        print(f"Kernel Size: {kernel_size}x{kernel_size}, 2D Time: {time_2d:.6f}s, 1D x2 Time: {time_1d:.6f}s")

    # Save results to a CSV file
    save_results_to_csv("gaussian_filter_kernel_sizes.csv", results_kernel_sizes,
